...
const TripsList = ({ tripsData }) => {
    ...
    return (
        <div className="mt-6 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-1 lg:grid-cols-2 xl:gap-x-8">
            {tripsData.map((trip) => (
                <div
                    className="group relative cursor-pointer flex flex-col lg:flex-row shadow px-4 py-4"
                    key={trip.tripid}
                >
                    {/* Image Section */}
                            ...
                            <div>
                                ...
                                <div className="flex flex-col gap-2">
                                    <div className="flex mt-1">
                                        ...
                                        <p className="text-xs font-medium text-neutral-900 mx-2">
                                    ...
                                    <div className="flex mt-1">
                                        ...
                                        <p className="text-xs font-medium text-neutral-900 mx-2">
                                            {formatDateTime(new Date(trip.endtime))}
                                        </p>
                                    </div>
                                    ...
                                    <div className="flex">
                                        <span
                                            className={`text-sm font-medium me-2 px-2.5 py-0.5 rounded dark:text-red-300 ${trip.status === 'Approved' ? 'bg-green-100 text-green-800 dark:bg-green-900' :
                                                    trip.status === 'Requested' ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900' :
                                                        trip.status === 'Started' ? 'bg-blue-100 text-blue-800 dark:bg-blue-900' :
                                        ...
                                        {trip.swapDetails && trip.swapDetails.length > 0 && (
                                            <span className="mx-4 inline-flex items-center rounded-md bg-yellow-50 px-1 py-1 text-xs font-medium text-yellow-800 ring-1 ring-inset ring-yellow-600/20">Swap Requestd</span>
                                        )}
